...
#endif
{
    ...
    if (set_pin_status(S_ON, HAZARD_BUTTON_PIN) == ECU_FAIL)
    {
    ...
    if (ret == ECU_SUCCESS)
    {
        ...
        if (set_pin_status(S_ON, ENGINE_REB_MODE) == ECU_FAIL)
        {
    ...
    if (ret == ECU_SUCCESS)
    {
        ...
        if (set_pin_status(S_ON, REB_IPC_WARNING) == ECU_FAIL)
        {
...
#endif
{
    ...
    // change pin 1 to off
    if (set_pin_status(S_OFF, HAZARD_BUTTON_PIN) == ECU_FAIL)
    {
    ...
    if (ret == ECU_SUCCESS)
    {
        ...
        if (set_pin_status(S_OFF, ENGINE_REB_MODE) == ECU_FAIL)
        {
    ...
    if (ret == ECU_SUCCESS)
    {
        ...
        if (set_pin_status(S_OFF, REB_IPC_WARNING) == ECU_FAIL)
        {
...
{
    uint8_t ret = ECU_SUCCESS;
    if (read_pin_status(status, HAZARD_BUTTON_PIN) == ECU_FAIL)
    {
...
{
    uint8_t ret = ECU_SUCCESS;
    if (set_pin_status(status, HAZARD_LIGHTS_PIN) == ECU_FAIL)
    {
...
{
    uint8_t ret = ECU_SUCCESS;
    if (read_pin_status(status, REB_ACTIVATE_PIN) == ECU_FAIL)
    {
        show_error("ECU.read_pin_status FAIL\n");
        ret = ECU_FAIL;
    }
...
{
    uint8_t ret = ECU_SUCCESS;
    if (set_pin_status(status, REB_ACTIVATE_PIN) == ECU_FAIL)
    {
        show_error("ECU.set_pin_status FAIL\n");
        ret = ECU_FAIL;
    }
...
{
    uint8_t ret = ECU_SUCCESS;
    if (set_pin_status(status, REB_DEACTIVATE) == ECU_FAIL)
    {
        show_error("ECU.set_pin_status FAIL\n");
        ret = ECU_FAIL;
    }
...
{
    uint8_t ret = ECU_SUCCESS;
    if (read_pin_status(status, REB_DEACTIVATE) == ECU_FAIL)
    {
        show_error("ECU.set_pin_status FAIL\n");
        ret = ECU_FAIL;
    }
...
{
    uint8_t ret = ECU_SUCCESS;
    can_frame frame = {.can_id = TCU_REB_ID, .can_dlc = 8, .data = {0}};
...
uint8_t handle_ipc_can(const unsigned char *data)
{
    ...
    if (signalREB == 0x01U)
    {
        ...
        if (set_reb_warning(S_ON) == ECU_FAIL)
        {
    ...
    if (ret == ECU_SUCCESS)
    {
        ...
        if (signalREB == 0x02U)
        {
            ...
            if (set_reb_warning(S_OFF) == ECU_FAIL)
            {
                show_error("bloc_engine FAIL\n");
                ret = ECU_FAIL;
            }
...
uint8_t set_reb_warning(uint8_t status)
{
    uint8_t ret = ECU_SUCCESS;
    // Remove driver notification (pin 3)
    if (set_pin_status(status, REB_IPC_WARNING) == ECU_FAIL)
    {
        show_error("ECU.set_pin_status FAIL  (driver notification)\n");
        ret = ECU_FAIL;
    }
